...
Native Codex `SubagentStart` supplies a bounded bootstrap with role, shared revision, task-selection status, rule entry paths and existing skill entries. A parent task named without dispatch text is only a candidate; select the explicit task before loading its full material. Other platform hooks may supply task material. Native `SubagentStart` injection is preferred; child-side loading is the fallback when injection is unavailable or truncated. Discover FastCtx once for local files, use it when available, and record a concrete failure or unavailability before falling back. Read one selected rule owner through EOF, continue partial output, and reuse fully read owners at the same revision. Before the first write or after changing task/worktree, run `worktree.py inspect <task> --path <lane> --for-write --owner <dispatch-owner> --json` with the actual task, assigned lane and dispatched owner; proceed only on success.
