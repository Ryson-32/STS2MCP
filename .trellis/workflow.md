...
<!--
  ...
  INVARIANT (validated by the selected-source profile fixture):
...
Read the active task and relevant specs. Delegate bounded implementation when it helps; always name the active task, role, objective, starting baseline, assigned worktree and owner, protected shared state, expected result, and checks. Production-writing workers use the shared isolated-worktree contract; read-only work does not need a worktree. Native `SubagentStart` injection is preferred; child-side loading is the fallback. Resolve the explicit task before reading material; native Codex bootstrap gives bounded entry points and treats a parent task as a candidate. Before the first write or after changing task/worktree, run `worktree.py inspect <task> --path <lane> --for-write --json` in the assigned repository and require the actual task, lane, Git and managed-worktree checks to succeed.
...
- **Recommended dispatch form**: `Active task: none` for explicit no-task read-only delivery, or `Active task: <task path>` to keep genuine task context and a validated durable-output boundary; a clearly assigned verified absolute task path is also valid
...
When delegation helps, spawn `trellis-implement`. The dispatch prompt starts with `Active task: <task path>` and then gives the role, bounded objective, starting baseline, assigned worktree, protected shared state, expected result, and necessary checks. Production-code writers default to their own short-lived worktree and build directory; exact file ownership is not a launch prerequisite. Tell the spawned role it is already the implementer and must not recursively dispatch another implement/check role. Roles remain unchanged on follow-up: research-to-implementation and author-to-independent-review require a new corresponding role.

Native Codex `SubagentStart` supplies a bounded bootstrap with role, shared revision, task-selection status, rule entry paths and existing skill entries. A parent task named without dispatch text is only a candidate; select the explicit task before loading its full material. Other platform hooks may supply task material. Native `SubagentStart` injection is preferred; child-side loading is the fallback when injection is unavailable or truncated. Discover FastCtx once for local files, use it when available, and record a concrete failure or unavailability before falling back. Read one selected rule owner through EOF, continue partial output, and reuse fully read owners at the same revision. Before the first write or after changing task/worktree, run `worktree.py inspect <task> --path <lane> --for-write --json` with the actual task and assigned lane as command cwd; proceed only when the task, lane, Git and managed-worktree checks succeed.
...
- Ordinary small changes may be author-checked. Reviewer selection, participation boundaries, review input and repair follow-up follow the [shared independent-review contract](https://github.com/Ryson-32/trellis-shared-spec/blob/main/spec/shared/subagent-orchestration.md#独立复核与完成报告), resolved through the shared snapshot loaded for the current step. A different model family is preferred when available and suitable, but it is not a qualification or completion gate.
