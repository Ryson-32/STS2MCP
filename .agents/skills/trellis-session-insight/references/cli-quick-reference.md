...
- **`mem` reads platform-owned JSONL and SQLite stores locally.** Devin CLI, OpenCode and ZCode use read-only SQLite snapshots; the other supported platforms use their session files. If the user has cleared the platform's session storage, `mem` cannot recover what is no longer on disk.
- **`mem` is read-only.** No remote sync and no edits to platform session stores. Any write you do based on `mem` findings is your own follow-up call into the editing tools available to you.
